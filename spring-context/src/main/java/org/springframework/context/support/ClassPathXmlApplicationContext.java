...
public class ClassPathXmlApplicationContext extends AbstractXmlApplicationContext {
	 ...
	 * @see #refresh()
	 */
	//解析ClassPathXmlApplicationContext构造函数：
	public ClassPathXmlApplicationContext(String[] configLocations, boolean refresh, @Nullable ApplicationContext parent) throws BeansException {
		//一直调用父类构造，直到 AbstractApplicationContext，设置一个空的 ApplicationContext 对象
		super(parent);
		//set config location(设置配置文件路径)
		 /*设置spring配置文件路径，并初始化抽象父类AbstractRefreshableConfigApplicationContext的configLocations参数，
	     *configLocations参数是一个String数组，存储多个xml文件路径，这个方法比较简单，朋友们自己看就行
	     */
		setConfigLocations(configLocations);
		//whether to automatically refresh the context
		//默认为true
		//重点来啦，refresh方法将会把我们所有IoC的初始化完成，包括Bean的初始化等操作
		if (refresh) {
			//主要的加载bean的过程
			refresh();
		}
