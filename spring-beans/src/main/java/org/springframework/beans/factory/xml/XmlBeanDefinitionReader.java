...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	 ...
	 */
	//标识validation mode:关闭
	public static final int VALIDATION_NONE = XmlValidationModeDetector.VALIDATION_NONE;

	/**
	 * Indicates that the validation mode should be detected automatically.
	 */
	//标识validation mode:自动检测
	public static final int VALIDATION_AUTO = XmlValidationModeDetector.VALIDATION_AUTO;

	/**
	 * Indicates that DTD validation should be used.
	 */
	//标识validation mode: DTD Validation
	public static final int VALIDATION_DTD = XmlValidationModeDetector.VALIDATION_DTD;

	/**
	 * Indicates that XSD validation should be used.
	 */
	//标识validation mode: XSD Validation
	...
	//自动检测验证模式 Auto_Validation
	private int validationMode = VALIDATION_AUTO;

	//是否是NameSpace（命名空间）感知
	private boolean namespaceAware = false;

	//Document Reader Class(文档读取类)的(Class)类类型
	...
	//当前已被加载的Xml bean definition resource
	...
	public int loadBeanDefinitions(Resource resource) throws BeanDefinitionStoreException {
		//使用EncodedResource进行包装，主要处理Stream流的charset字符编码问题
	...
	public int loadBeanDefinitions(EncodedResource encodedResource) throws BeanDefinitionStoreException {
		...
		//从ThreadLocal获取本线程已经加载的resource
		...
		try {
			...
			try {
				//通过InputStream构造xml.sax的InputSource对象
	...
	protected int doLoadBeanDefinitions(InputSource inputSource, Resource resource) throws BeanDefinitionStoreException {
		try {
			//1. 通过InputSource和Resource加载Document
			Document doc = doLoadDocument(inputSource, resource);
			//2. 通过加载的Document和Resource注册Bean
	...
	protected Document doLoadDocument(InputSource inputSource, Resource resource) throws Exception {
		return this.documentLoader.loadDocument(inputSource, getEntityResolver(), this.errorHandler,
				//getValidationModeForResource获取xml配置文件的验证模式
	 ...
	 */
	//根据Resource获取Validation Mode（验证模式）
	protected int getValidationModeForResource(Resource resource) {
		//获取默认的: VALIDATION_AUTO 自动验证模式
		int validationModeToUse = getValidationMode();
		if (validationModeToUse != VALIDATION_AUTO) {
			//不是VALIDATION_AUTO,则使用用户指定的验证模式
	...
	public int registerBeanDefinitions(Document doc, Resource resource) throws BeanDefinitionStoreException {
		...
		//2. 获取已经注册Bean的count数量
		int countBefore = getRegistry().getBeanDefinitionCount();
		//3. 处理注册Bean（委派给DocumentReader进行处理）
		documentReader.registerBeanDefinitions(doc, createReaderContext(resource));
		//4. 当前注册的Bean的count数量-注册前的count
