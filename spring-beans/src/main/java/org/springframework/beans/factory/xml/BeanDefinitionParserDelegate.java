...
public class BeanDefinitionParserDelegate {
	...
	/**
	 * Parse the bean definition itself, without regard(留意，注意) to name or aliases. May return
	 * {@code null} if problems occurred during the parsing of the bean definition.
	 */
	@Nullable
	//解析<bean>是使用栈(Stack)来进行push/pop操作的
	public AbstractBeanDefinition parseBeanDefinitionElement(Element ele, String beanName, @Nullable BeanDefinition containingBean) {
		...
		try {
			...
			//对当前的<bean>元素中配置的一些属性进行解析并设置（包括singleton，scope，abstract,lazy-init，autowire,depends-on,autowire-candidate等）
			...
			//-------------------------------------------------------------------------//
			//   Constructor-arg（构造函数）、Property（属性）、Qualifier（限定名）解析都是【栈】操作
			//-------------------------------------------------------------------------//
			//对<bean>元素的构造方法设置
			parseConstructorArgElements(ele, bd);
			//对<bean>元素的property设置，property这个配置比较常见
	...
	public AbstractBeanDefinition parseBeanDefinitionAttributes(Element ele, String beanName,
        ...
        //spring 1.x "singleton" 属性 upgrade（升级）scope属性
		//获取spring 1.x中singleton属性，upgrade升级后为:scope
		...
		//获取<bean>中的abstract属性
		if (ele.hasAttribute(ABSTRACT_ATTRIBUTE)) {
			//设置abstract属性
			bd.setAbstract(TRUE_VALUE.equals(ele.getAttribute(ABSTRACT_ATTRIBUTE)));
		}
        //获取<bean>的lazy-init属性值
		...
		//获取<bean>的autowire-candidate属性
		//autowire-candidate="false" 表示该对象不参与自动注入
		String autowireCandidate = ele.getAttribute(AUTOWIRE_CANDIDATE_ATTRIBUTE);
		if ("".equals(autowireCandidate) || DEFAULT_VALUE.equals(autowireCandidate)) {
			//获取default的autowireCandidate
			String candidatePattern = this.defaults.getAutowireCandidates();
			if (candidatePattern != null) {
				//用,分隔candidatePattern
	...
	*/
	public void parseConstructorArgElements(Element beanEle, BeanDefinition bd) {
		...
		for (int i = 0; i < nl.getLength(); i++) {
			Node node = nl.item(i);
			//判断是<constructor-arg />标签
			if (isCandidateElement(node) && nodeNameEquals(node, CONSTRUCTOR_ARG_ELEMENT)) {
				//节ConstructorArg节点<constructor-arg />
	...
	public void parseLookupOverrideSubElements(Element beanEle, MethodOverrides overrides) {
		...
		for (int i = 0; i < nl.getLength(); i++) {
			Node node = nl.item(i);
			//判断是<lookup-method />标签
			if (isCandidateElement(node) && nodeNameEquals(node, LOOKUP_METHOD_ELEMENT)) {
				Element ele = (Element) node;
				//获取<lookup-method />的name属性
				String methodName = ele.getAttribute(NAME_ATTRIBUTE);
				//获取<lookup-method>的bean属性
    ...
    */
	public void parseReplacedMethodSubElements(Element beanEle, MethodOverrides overrides) {
		...
		for (int i = 0; i < nl.getLength(); i++) {
			Node node = nl.item(i);
			//判断是<replaced-method />标签
			if (isCandidateElement(node) && nodeNameEquals(node, REPLACED_METHOD_ELEMENT)) {
				Element replacedMethodEle = (Element) node;
				//获取<replaced-method />的name属性
				String name = replacedMethodEle.getAttribute(NAME_ATTRIBUTE);
				//获取<replaced-method />的replacer属性
...
/*	 <bean id="liangyuqi" class="models.Student">
	...
	public void parseConstructorArgElement(Element ele, BeanDefinition bd) {
		//获取<constructor-arg />的index属性
		String indexAttr = ele.getAttribute(INDEX_ATTRIBUTE);
		//获取<constructor-arg />的type属性
		String typeAttr = ele.getAttribute(TYPE_ATTRIBUTE);
		//获取<constructor-arg />的name属性
		String nameAttr = ele.getAttribute(NAME_ATTRIBUTE);
		//<constructor-arg />中有index属性，则按着index值的顺序，否则默认顺序
			...
			try {
				...
				if (index < 0) {
					...
					try {
						...
						//解析构造方法中<constructor-arg />中的property value属性值
	...
	*/
	public void parseQualifierElement(Element ele, AbstractBeanDefinition bd) {
		//获取<qualifier />的type属性值
		String typeName = ele.getAttribute(TYPE_ATTRIBUTE);
		if (!StringUtils.hasLength(typeName)) {
			error("Tag 'qualifier' must have a 'type' attribute", ele);
			return;
		}
		//记录处理的当前qualifier,压入栈push()
		this.parseState.push(new QualifierEntry(typeName));
		try {
			AutowireCandidateQualifier qualifier = new AutowireCandidateQualifier(typeName);
			qualifier.setSource(extractSource(ele));
			//获取<qualifier />的value属性值
	 ...
	 */
	//<constructor-arg />和<property />解析属性值(Property)value
	@Nullable
	public Object parsePropertyValue(Element ele, BeanDefinition bd, @Nullable String propertyName) {
		//propertyName不等于空则是<property />
		//propertyName等于空则是<constructor-arg />
		String elementName = (propertyName != null ? "<property> element for property '" + propertyName + "'" : "<constructor-arg> element");

		// Should only have one child element: ref, value, list, etc.
		//应该只有一个子元素element: ref、value、list 等等.
		NodeList nl = ele.getChildNodes();
		Element subElement = null;
		for (int i = 0; i < nl.getLength(); i++) {
			Node node = nl.item(i);
			//不是<description />和<meta />标签
